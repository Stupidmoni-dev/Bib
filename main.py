...
# Bible Verse command
async def verse(update: Update, context: CallbackContext):
    bible_verse = get_bible_verse()
    await update.message.reply_text(bible_verse)
...
async def answer(update: Update, context: CallbackContext):
    ...
    if user_answer.lower() == correct_answer.lower():
        await update.message.reply_text("Correct! You earned 1 point.")
    else:
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if "bible" in user_message or "christian" in user_message:
        await update.message.reply_text("The Bible is God's word and it contains teachings for a righteous life. Feel free to ask about any verses or topics.")
    
    elif "love" in user_message:
        await update.message.reply_text("Love is the greatest commandment. Jesus said, 'Love your neighbor as yourself.'")

    elif "god" in user_message:
        await update.message.reply_text("I am a man who loves God but chooses to stay at home to worship Him, condemned as a pagan because I refuse to go to church.")

    else:
        await update.message.reply_text("I am here to share knowledge about the Bible and Christian life. Ask me anything!")
